/**
 * Keyboard handling for Arculator WASM. 
...
function browser_init_keys(keystate, keylayout) {
    ...
    class EmulatorKeys
    {
        constructor(browserKeyMap = null, keyStatePtr, keyLayoutPtr) {
        ...
        installEventHandlers() {
            let obj = this;
            let captureElement = document.getElementById('canvas');
            captureElement.addEventListener('keydown', (evt) => obj.handleKeyDownEvent(evt));
            captureElement.addEventListener('keyup', (evt) => obj.handleKeyUpEvent(evt));
            captureElement.addEventListener('mousedown', (evt) => obj.handleMouseButtonEvent(evt));
    ...
    let emukeys = new EmulatorKeys(BROWSER_KEY_MAP_UK, keystate, keylayout);
    emukeys.installEventHandlers();
    window.emukeys = emukeys;
}
...
function browser_init_mouse(mxPtr, myPtr, buttonsPtr, mouseModePtr) { 
    ...
    mxPtr = mxPtr >> 2;
    myPtr = myPtr >> 2;
    buttonsPtr = buttonsPtr >> 2;
    mouseModePtr = mouseModePtr >> 2;
    window.emukeys.buttonsPtr = buttonsPtr;

    const canvas = document.getElementById('canvas');
    const canvasPaddingX = (canvas.clientWidth - canvas.width)/2;
    const canvasPaddingY = (canvas.clientHeight - canvas.height)/2;
    const CANVAS_SNAP_BORDER = 50;

    let cx = 0;
    let cy = 0;

    function sendAbsMouse(x,y) {
        HEAP32[mouseModePtr] = 2;
        HEAP32[mxPtr] = x;
        // invert Y mouse coord
        HEAP32[myPtr] = canvas.height - y;
        cx = x;
        cy = y;
    }

    canvas.addEventListener('mousemove', evt => {
        if (document.pointerLockElement || window.emukeys.mouseCaptureNeeded) {
            HEAP32[mxPtr] += evt.movementX;
            HEAP32[myPtr] += evt.movementY;
            HEAP32[mouseModePtr] = 1;
        } else if (!window.emukeys.mouseCaptureNeeded) {
    ...
    canvas.addEventListener('mouseleave', evt => {
        ...
        if (window.emukeys.mouseCaptureNeeded)
            return;
        ...
        if (x == 0 || y == 0 || x == canvas.width || y == canvas.height)
            return;
        if (x < CANVAS_SNAP_BORDER) x = 0;
        if (x > canvas.width - CANVAS_SNAP_BORDER)  x = canvas.width;     
        if (y < CANVAS_SNAP_BORDER) y = 0;
        if (y > canvas.height - CANVAS_SNAP_BORDER) y = canvas.height;
        sendAbsMouse(x,y);
    });
}
...
function notify_mouse_lock_required(needed) {
    ...
    window.emukeys.mouseCaptureNeeded = needed;
